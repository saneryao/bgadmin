			...
			<div class="modal-body lead text-center no-padding" id="info-content">
				...
				<fieldset>
					...
					<div class="row no-padding">
						...
						<div class="col-sm-8">
							<input type="text" name="name" class="form-control" maxlength="32" placeholder="{{i18n .Lang "username"}}" required minlength="4" />
						</div>
					</div>
	...
	jQuery(function($) {
		$("#form-info").validate({  
			...
			submitHandler:function(form) {
				submit_ajax($("#form-info"), submit_callback);
				return false;
			}
		});
	});
	...
	$('.page-content-area').ace_ajax('loadScripts', scripts, function() {
		...
		jQuery(function($) {
			...
			$('#dynamic_table tbody').on( 'click', 'a#edit', function () {
				...
				$('select[name="state"]').each( function() {  // 遍历所有option
