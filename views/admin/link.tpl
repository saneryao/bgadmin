		...
		<div class="modal-content">
			...
			<div class="modal-body lead text-center no-padding" id="info-content">
				...
				<fieldset>
					...
					<div class="row">
						<div class="col-sm-2 col-sm-offset-1">
							<label>{{i18n .Lang "url"}}<span class="red">*</span></label>
						</div>
						<div class="col-sm-8">
	...
	$('.page-content-area').ace_ajax('loadScripts', scripts, function() {
		...
		jQuery(function($) {
			...
			$('#dynamic_table tbody').on( 'click', 'a#edit', function () {
				...
				var data = myRow.data();
				$('#form-info').attr('action', '{{urlfor "LinkAPI.Put"}}/'+data.id);
			...
			$('#dynamic_table tbody').on( 'click', 'a#del', function () {
				if (confirm('确定删除吗？')) {
					...
					$.ajax({
						url: '{{urlfor "LinkAPI.Delete"}}/' + data['id'],
						type: 'DELETE',
			...
			setTimeout(function() {
				myTable.button(0).action(function (e, dt, button, config) {
					$('#form-info').attr('action', '{{urlfor "LinkAPI.Post"}}');
